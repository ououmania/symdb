...
void Session::read_header() {
    boost::asio::async_read(socket_,
        boost::asio::buffer((char*) &req_header_, sizeof(req_header_)),
...
void Session::handle_read_header(const boost::system::error_code& err)
{
    ...
    boost::asio::async_read(socket_,
        boost::asio::buffer(req_body_), boost::asio::transfer_exactly(req_body_.size()),
...
void Session::do_write(const google::protobuf::Message &head_pb,
    ...
    boost::asio::async_write(socket_, reply_,
        ...
        [total_size,self](boost::system::error_code ec, std::size_t /* length */)
        {
            if (!ec)
            {
                self->read_header();
...
void Session::get_symbol_definition(const uint8_t *buffer, size_t length)
{
    ...
    ResponseGuard<GetSymbolDefinitionRsp> rsp(this, MessageID::GET_SYMBOL_DEFINITION_RSP);
    ProjectPtr project = ServerInst.GetProject(msg.proj_name());
    if (!project) {
        LOG_ERROR << kErrorProjectNotFound << ", project=" << msg.proj_name();
        rsp->set_error(kErrorProjectNotFound);
    ...
    if (!msg.abs_path().empty()) {
        Location location = project->QuerySymbolDefinition(msg.symbol(), msg.abs_path());
        if (location.IsValid()) {
            location.Seriaize(*rsp->add_locations());
            LOG_DEBUG << "project=" << msg.proj_name() << ", symbol=" << msg.symbol()
                      << ", abs_path=" << msg.abs_path() << ", path=" << location.filename();
        } else {
...
void Session::list_file_symbols(const uint8_t *buffer, size_t length)
{
    ...
    LOG_DEBUG << "project=" << msg.proj_name() << ", rel_path=" << msg.relative_path();
