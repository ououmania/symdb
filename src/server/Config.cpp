...
inline std::string child_value_or_default(const pugi::xml_node &node,
...
inline std::string child_value_or_throw(const pugi::xml_node &node,
                                        const char *child_name) {
  const pugi::xml_node &child = node.child(child_name);
  if (!child) {
    THROW_AT_FILE_LINE("node<%s> has no child<%s>", node.name(), child_name);
  }
  return child.child_value();
}

RegexPattern::RegexPattern(const std::string &orig_pattern,
                           const std::string &used_pattern, bool is_from_global)
...
void ProjectConfig::SetBuildPath(std::string path) {
  LOG_DEBUG << "project=" << name_ << " build_path=" << path;
...
void ProjectConfig::SetCmakeFile(std::string path) {
  ...
  LOG_DEBUG << "project=" << name_ << " final_cmake_file=" << cmake_file;
}
...
void Config::InitGlobalExcludePattern(const pugi::xml_node &root_node) {
  auto global_exclude_entry =
      root_node.select_nodes("//GlobalExcluded/ExcludeEntry");
...
void Config::InitProjectsConfig(const pugi::xml_node &root) {
  ...
  for (const auto &proj : project_nodes) {
    ...
    pc->is_enable_file_watch(
        node.child("EnableFileWatch").text().as_bool(true));
