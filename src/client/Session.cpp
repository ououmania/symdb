...
Session::Session(boost::asio::io_service &io_context, const std::string &path)
  ...
  tv.tv_sec = 5;
  tv.tv_usec = 0;
  if (setsockopt(socket_.native_handle(), SOL_SOCKET, SO_RCVTIMEO, &tv,
                 sizeof(tv)) != 0) {
    throw std::runtime_error(strerror(errno));
  }
}

void Session::create_project(const std::string &proj_name,
...
void Session::list_file_symbols(const std::string &proj_name,
...
bool Session::send(int msg_id, const google::protobuf::Message &body) {
  ...
  boost::asio::write(socket_, buf, boost::asio::transfer_at_least(total_size),
                     error);
  if (error) {
